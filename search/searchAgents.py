...
class FoodSearchProblem:
    ...
    def getSuccessors(self, state):
        ...
        successors = []
        self._expanded += 1 # DO NOT CHANGE
        for directions in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            x,y = state[0]
            dx, dy = Actions.directionToVector(directions)
            nextx, nexty = int(x + dx), int(y + dy)
            if not self.walls[nextx][nexty]:
                succ_food = state[1].copy()
                succ_food[nextx][nexty] = False
                successors.append((((nextx,nexty),succ_food), directions, 1))
        return successors
    ...
    def getCostOfActions(self, actions):
        ...
        for action in actions:
            dx, dy = Actions.directionToVector(action)
            x, y = int(x + dx), int(y + dy)
            # figure out whether it's illegal
            if self.walls[x][y]:
                return 999999
            cost += 1
        return cost
        ...
        while Q: 
            ...
            if not node in visited:
                ...
                for successor in successors:
                    ...
                    Q.push((coordinate, actions + [direction]))
